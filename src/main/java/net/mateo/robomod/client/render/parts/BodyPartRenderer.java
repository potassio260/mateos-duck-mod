...
public class BodyPartRenderer extends CyborgPartRenderer {
    ...
    public void render(PlayerModel<?> contextModel, PoseStack poseStack,
        ...
        this.model.get().getBody().copyFrom(contextModel.body);
        contextModel.body.visible = false;

        VertexConsumer vertexConsumer = bufferSource.getBuffer(
                RenderType.entityCutout(new ResourceLocation(RoboMod.MOD_ID, this.texture)));
        this.model.get().getBody().render(poseStack, vertexConsumer, packedLight, OverlayTexture.NO_OVERLAY);
    }
    ...
    public void renderAssembler(AssemblerBlockEntity assembler, BlockState state, float tickDelta,
        ...
        poseStack.pushPose();
        this.model.get().getBody().resetPose();
        this.model.get().getBody().y = -24;
